...
from app.db.database import Clip, get_session
from app.services.audio_service import generate_single_voiceover

router = APIRouter()
@router.put("/modify_clip/{clip_id}")
async def modify_clip(clip_id: int, prompt: str = None, code : str = None,narration_text: str = None,visuals: str = None, session: Session = Depends(get_session)):
    ...
    result = coding_agent_compiled.run(intial_state)
